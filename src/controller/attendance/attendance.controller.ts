// // src/controller/attendance/attendance.controller.ts
...
//   }
//   return (
//     Object.entries(map).find(([, v]) => v.in && !v.out)?.[0] ?? null
//   );
// }
...
//       if (!log.hasOpenSession)
//         throw Object.assign(
//           new Error("No open check-in session found."),
//           { statusCode: 409 }
//         );
...
//       res, 500, err?.message ?? "Failed to fetch status"
//     );
//   }
// }
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to fetch today's attendance"
//     );
//   }
// }
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to fetch attendance history"
//     );
//   }
// }
...
//       return sendErrorResponse(
//         res, 400, "type, startDate and reason are required"
//       );

//     const validTypes = Object.values(LeaveType);
//     if (!validTypes.includes(type))
//       return sendErrorResponse(
//         res, 400, `type must be one of: ${validTypes.join(", ")}`
//       );

//     if (type === LeaveType.MULTI_DAY && !endDate)
//       return sendErrorResponse(
//         res, 400, "endDate is required for MULTI_DAY leave"
//       );
...
//     if (conflict)
//       return sendErrorResponse(
//         res, 409, "Overlapping leave request already exists"
//       );
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to apply leave"
//     );
//   }
// }
...
//         return sendErrorResponse(
//           res, 400, `status must be one of: ${validStatuses.join(", ")}`
//         );
//       where.status = status;
//     }
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to fetch leaves"
//     );
//   }
// }
...
//       return sendErrorResponse(
//         res, 409, "Only PENDING leave requests can be cancelled"
//       );
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to cancel leave"
//     );
//   }
// }
...
//       present:    overview.filter((o) => o.status === AttendanceStatus.PRESENT).length,
//       halfDay:    overview.filter((o) => o.status === AttendanceStatus.HALF_DAY).length,
//       absent:     overview.filter((o) => o.status === AttendanceStatus.ABSENT).length,
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to fetch attendance"
//     );
//   }
// }
...
//         (s, l) => s + l.totalWorkMinutes, 0
//       );
//       return {
//         account: accLogs[0].account,
//         totalDays:        accLogs.length,
//         present:  accLogs.filter((l) => l.status === AttendanceStatus.PRESENT).length,
//         halfDay:  accLogs.filter((l) => l.status === AttendanceStatus.HALF_DAY).length,
//         absent:   accLogs.filter((l) => l.status === AttendanceStatus.ABSENT).length,
//         holiday:  accLogs.filter((l) => l.status === AttendanceStatus.HOLIDAY).length,
//         totalWorkMinutes,
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to generate summary"
//     );
//   }
// }
...
//     if (!checkedAtStr) return sendErrorResponse(res, 400, "checkedAt is required");
...
//     const date      = dateStr ? toDateOnly(new Date(dateStr)) : toDateOnly(checkedAt);
...
//       return sendErrorResponse(res, err.statusCode, err.message);
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to record check-in"
//     );
//   }
// }
...
//       if (!log)
//         throw Object.assign(
//           new Error("No attendance log for this date."),
//           { statusCode: 404 }
//         );
...
//         providedSessionId ?? findOpenSessionId(allChecks);
...
//       if (sessionOutExists)
//         throw Object.assign(
//           new Error("Session already has a CHECK_OUT."),
//           { statusCode: 409 }
//         );
...
//           !allChecks.some(
//             (co) =>
//               co.sessionId === c.sessionId &&
//               co.type === CheckType.CHECK_OUT
//           )
//       );
...
//       res, 201, "Manual check-out recorded", result
//     );
//   } catch (err: any) {
//     if (err?.statusCode)
//       return sendErrorResponse(res, err.statusCode, err.message);
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to record check-out"
//     );
//   }
// }
...
//       return sendErrorResponse(
//         res, 400, "Provide at least status or overrideNote"
//       );
...
//       if (!validStatuses.includes(status))
//         return sendErrorResponse(
//           res, 400, `Invalid status. Must be one of: ${validStatuses.join(", ")}`
//         );
//     }
...
//       { date: log.date, status: updated.status, overrideNote: updated.overrideNote }
//     );

//     return sendSuccessResponse(res, 200, "Attendance overridden", updated);
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to override attendance"
//     );
//   }
// }
...
//           if (c.type === CheckType.CHECK_IN)  sessions[c.sessionId].in  = c.checkedAt;
//           if (c.type === CheckType.CHECK_OUT) sessions[c.sessionId].out = c.checkedAt;
//         }
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to delete check log"
//     );
//   }
// }
...
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to fetch leave requests"
//     );
//   }
// }
...
//     if (!status || (status !== LeaveStatus.APPROVED && status !== LeaveStatus.REJECTED))
//       return sendErrorResponse(
...
//     if (leave.status !== LeaveStatus.PENDING)
//       return sendErrorResponse(
//         res, 409, "Leave request is no longer PENDING"
//       );
...
//     return sendSuccessResponse(
//       res, 200, `Leave ${status.toLowerCase()}`, updated
//     );
//   } catch (err: any) {
//     return sendErrorResponse(
//       res, 500, err?.message ?? "Failed to decide leave"
//     );
//   }
// }